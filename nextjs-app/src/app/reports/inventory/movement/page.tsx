...
import { Download, ArrowUpRight, ArrowDownRight, RefreshCw, Package, Loader2, FileSpreadsheet, Printer } from 'lucide-react';
...
export default function StockMovementPage() {
    ...
    const handleDownloadPdf = () => {
        ...
        downloadPdf(`${endpoints.reports.inventoryMovementPdf}?${pdfParams}`, 'stock-movement-report');
    };

    const handleExportCsv = () => {
        if (!report?.products) return;
        exportToCsv(report.products.map(p => ({
            product_name: p.product_name,
            incoming_cartons: p.incoming.cartons,
            incoming_weight: p.incoming.weight,
            outgoing_cartons: p.outgoing.cartons,
            outgoing_weight: p.outgoing.weight,
            net_cartons: p.net_change.cartons,
            net_weight: p.net_change.weight,
        })), 'stock-movement', [
            { key: 'product_name', header: 'Product' },
            { key: 'incoming_cartons', header: 'In (Cartons)' },
            { key: 'incoming_weight', header: 'In (kg)' },
            { key: 'outgoing_cartons', header: 'Out (Cartons)' },
            { key: 'outgoing_weight', header: 'Out (kg)' },
            { key: 'net_cartons', header: 'Net (Cartons)' },
            { key: 'net_weight', header: 'Net (kg)' },
        ]);
    };
